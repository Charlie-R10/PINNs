...
def combined_loss(y_true, y_pred):
    ...
    total_loss = 5*trad_loss + pinn_loss_val # '5' is a scalable parameter - can change this to alter loss function
    return total_loss
...
def model_metrics(values):
    ...
    print(f'r^2 (PINN) = {r2_score(y_true, predicted_data)}')
    print(f'RMSE (PINN) = {np.sqrt(mean_squared_error(y_true, predicted_data))}')
    print(f'MAE (PINN) = {mean_absolute_error(y_true, predicted_data)}')
    print(f'MAE PERCENTAGE (PINN) = {mean_absolute_percentage_error(y_true, predicted_data) * 100000}')
    print(f'MAX ERROR (PINN) = {max_error(y_true, predicted_data) * 100000}')


model_metrics(predicted_pinn)
model_metrics(predicted_nn)
